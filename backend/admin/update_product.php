...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $price = isset($_POST['price']) ? trim($_POST['price']) : ''; // Lưu dưới dạng chuỗi
    ...
    $stmt = $conn->prepare("UPDATE products SET name = ?, price = ?, image = ?, category = ?, quantity = ? WHERE id = ?");
    if (!$stmt) {
        echo json_encode(['success' => false, 'message' => 'Lỗi chuẩn bị câu lệnh SQL: ' . $conn->error]);
        exit();
    }

    // Gán tham số (price giờ là string)
    $stmt->bind_param("ssssii", $name, $price, $image, $category, $quantity, $id);
